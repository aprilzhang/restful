 ...
 */
public final class TransactionResourceTest {
	...
	public void getTransactionSuccess() {
		...
		final Transaction found = RESOURCES.target("/transactions/1").request().get(Transaction.class);
	...
	public void createTransaction() throws JsonProcessingException {
		...
		verify(TRANSACTION_DAO).create(transactionCaptor.capture());
